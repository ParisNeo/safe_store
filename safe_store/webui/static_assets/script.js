...
function setupSocketIO() {
    socket = io({ path: '/sio' });
    socket.on('connect', () => { sessionId = socket.id; console.log("Socket.IO connected with session ID:", sessionId); });
    socket.on('progress_update', (data) => handleProgressUpdate(data));
    socket.on('disconnect', () => { console.log("Socket.IO disconnected."); sessionId = null; });
    socket.on('connect_error', (error) => { console.error("Socket.IO connection error:", error); sessionId = null; });
}
...
function handleProgressUpdate(data) {
    ...
    if (progress >= 1.0) {
        if (message.toLowerCase().includes("error")) {
             barElement.classList.remove('bg-blue-600'); barElement.classList.add('bg-red-600');
        } else {
             barElement.classList.remove('bg-blue-600'); barElement.classList.add('bg-green-600');
        }
...
function loadAppSettings() {
    ...
    if (storedSettings) {
        try { appSettings = { ...appSettings, ...JSON.parse(storedSettings) }; } catch (e) { console.error("Failed to parse stored settings:", e); }
    }
...
async function loadAndDisplayDatabases() {
    try {
        ...
        databases.forEach(db => {
            ...
            div.innerHTML = `
                <div>
                    <p class="font-semibold dark:text-gray-200">${db.name} ${db.is_active ? '<span class="text-green-500 text-sm">(Active)</span>' : ''}</p>
                    <p class="text-xs text-gray-500 dark:text-gray-400 font-mono" title="${db.db_file}">DB: ${db.db_file}</p>
                </div>
                <div class="space-x-2">
                    ${!db.is_active ? `<button data-db-name="${db.name}" class="activate-db-btn px-3 py-1 bg-green-500 hover:bg-green-600 text-white rounded text-sm">Activate</button>` : ''}
                    <button data-db-name="${db.name}" class="delete-db-btn px-3 py-1 bg-red-600 hover:bg-red-700 text-white rounded text-sm" ${databases.length <= 1 ? 'disabled title="Cannot delete the last database"' : ''}><i class="fas fa-trash"></i></button>
                </div>`;
    ...
    } catch (error) { databaseListContainer.innerHTML = `<p class="text-sm italic text-red-500">Error: ${error.message}</p>`; }
}
async function handleActivateDatabase(event) {
    const dbName = event.target.dataset.dbName;
    if (confirm(`Activate "${dbName}"? The app will reload.`)) {
        try {
            await apiRequest(API_ENDPOINT_DATABASE_ACTION(dbName, 'activate'), 'PUT');
            graphContainer.innerHTML = `<div class="p-4 text-lg flex items-center justify-center h-full"><i class="fas fa-sync fa-spin mr-3"></i> Reloading...</div>`;
...
async function handleDeleteDatabase(event) {
    ...
    if (confirm(`Delete config for "${dbName}"? This won't delete files.`)) {
        try {
...
createDbForm?.addEventListener('submit', async (event) => {
    ...
    if (!name) { createDbError.textContent = "Name is required."; return; }
    try {
        await apiRequest(API_ENDPOINT_DATABASES, 'POST', { name });
        closeModal(createDbModal);
        loadAndDisplayDatabases();
    } catch (error) { createDbError.textContent = `Error: ${error.message}`; }
});
...
fileInput?.addEventListener("change", () => {
    ...
    if (fileInput.files.length > 0) {
        Array.from(fileInput.files).forEach(file => { fileListPreview.innerHTML += `<div class="text-xs p-1 bg-gray-100 dark:bg-gray-700 rounded truncate">${file.name}</div>`; });
        uploadSubmitBtn.disabled = false;
    } else { uploadSubmitBtn.disabled = true; }
});
uploadForm?.addEventListener("submit", async (event) => {
    event.preventDefault();
    if (!fileInput.files.length || !sessionId) { showUserStatus(uploadOverallStatus, "Cannot upload: not connected.", "error"); return; }
    uploadSubmitBtn.disabled = true;
    const guidance = extractionGuidanceInput.value.trim();
    for (const file of fileInput.files) {
        const formData = new FormData();
        formData.append("file", file); formData.append("guidance", guidance); formData.append("sid", sessionId);
        try {
            const response = await apiRequest(API_ENDPOINT_UPLOAD, "POST", formData, true);
            const data = await response.json();
            showUserStatus(uploadOverallStatus, `Submitted '${file.name}' (Task: ${data.task_id}).`, "info", 6000);
        } catch (error) { showUserStatus(uploadOverallStatus, `Failed to submit '${file.name}': ${error.message}`, "error", 0); }
    }
    setTimeout(fetchGraphData, 3000); uploadSubmitBtn.disabled = false;
});
...
function openNodeEditModal(nodeData, visJsCallback) {
    ...
    saveNodeChanges = async () => {
        try {
            ...
            const updatedNode = await apiRequest(`${API_ENDPOINT_NODE}${nodeData.id}`, 'PUT', payload);
            const updatedVisData = { ...updatedNode, original_label: updatedNode.label };
            if (visJsCallback) visJsCallback(updatedVisData); else nodesDataSet.update(updatedVisData);
            closeModal(nodeEditModal);
            updateSelectionInfoPanel({ nodes: [nodeData.id], edges: [] });
        } catch (error) { alert(`Failed to update node: ${error}`); if (visJsCallback) visJsCallback(null); }
    };
...
function openEdgeEditModal(edgeData, visJsCallback) {
    ...
    saveEdgeChanges = async () => {
        try {
            ...
            const updatedEdge = await apiRequest(`${API_ENDPOINT_EDGE}${edgeData.id}`, 'PUT', payload);
            const updatedVisData = { ...updatedEdge, from: updatedEdge.from_node_id, to: updatedEdge.to_node_id };
            if (visJsCallback) visJsCallback(updatedVisData); else edgesDataSet.update(updatedVisData);
            closeModal(edgeEditModal);
            updateSelectionInfoPanel({ nodes: [], edges: [edgeData.id] });
        } catch (error) { alert(`Failed to update edge: ${error}`); if (visJsCallback) visJsCallback(null); }
    };
    openModal(edgeEditModal);
}
editSelectedNodeBtn?.addEventListener("click", () => {
    const nodeId = editSelectedNodeBtn.dataset.nodeId;
    if (nodeId && nodesDataSet.get(nodeId)) openNodeEditModal(nodesDataSet.get(nodeId), (data) => nodesDataSet.update(data));
});
editSelectedEdgeBtn?.addEventListener("click", () => {
    const edgeId = editSelectedEdgeBtn.dataset.edgeId;
    if (edgeId && edgesDataSet.get(edgeId)) openEdgeEditModal(edgesDataSet.get(edgeId), (data) => edgesDataSet.update(data));
});
...
async function handleGraphSearch(event) {
    ...
    clearSearch(false);
    try {
        const results = await apiRequest(`${API_ENDPOINT_GRAPH_SEARCH}?${new URLSearchParams({ q: query })}`);
        showUserStatus(graphActionStatus, `Found ${results.nodes.length} nodes.`, "success");
        populateSearchResults(results);
        searchResultsContainer.classList.remove("hidden");
    } catch (error) { showUserStatus(graphActionStatus, `Search failed: ${error.message}`, "error"); }
    finally { graphLoadingOverlay.classList.add('hidden'); }
}
function populateSearchResults(results) {
    searchResultsList.innerHTML = "";
    results.nodes.forEach(node => {
        const item = document.createElement("button");
        item.className = "w-full text-left p-1.5 text-xs rounded-md hover:bg-gray-200 dark:hover:bg-gray-600";
        item.dataset.type = "node"; item.dataset.id = node.id;
        item.innerHTML = `<i class="fas fa-circle mr-2 text-blue-400"></i> ${node.label.substring(0, 40)}`;
...
function handleSearchResultClick(event) {
    ...
    if (type === 'node') { network.focus(id, { animation: true, scale: 1.2 }); network.selectNodes([id]); }
}
function clearSearch(clearInput = true) {
    if (clearInput) graphSearchInput.value = "";
    searchResultsList.innerHTML = ""; searchResultsContainer.classList.add("hidden");
    if (network) network.unselectAll(); if(clearInput) showUserStatus(graphActionStatus, "", "info", 1);
}
...
async function fetchGraphData() {
    ...
    try {
        ...
        if (!network) renderGraph(); else network.setData({ nodes: nodesDataSet, edges: edgesDataSet });
        applyCurrentLayout();
        ...
        graphContainer.innerHTML = `<div class="p-4 text-red-600">Error: ${error.message}</div>`;
    } finally { graphLoadingOverlay.classList.add('hidden'); }
}
...
function renderGraph() {
    ...
    const options = {
        nodes: { shape: "dot", size: 15, font: { size: 13, color: themeColors.nodeFontColor }, borderWidth: 2, color: { border: themeColors.nodeBorder, background: themeColors.nodeBg, highlight: { border: themeColors.nodeHighlightBorder, background: themeColors.nodeHighlightBg }}},
        edges: { color: {color: themeColors.edgeColor, highlight: themeColors.nodeHighlightBorder }, width: 1.5, smooth: { type: 'continuous' }, arrows: { to: { enabled: true, scaleFactor: 0.6 }}, font: { size: 10, color: themeColors.edgeFontColor, strokeWidth: 2, strokeColor: themeColors.edgeStrokeColor, align: 'middle' }},
        interaction: { tooltipDelay: 150, navigationButtons: false, keyboard: { enabled: true, bindToWindow: false }, hover: true },
        physics: { enabled: appSettings.physicsOnLoad, solver: 'barnesHut', barnesHut: { gravitationalConstant: -25000, centralGravity: 0.05, springLength: 110, springConstant: 0.02, damping: 0.09, avoidOverlap: 0.2 }, stabilization: { iterations: 300, fit: true }},
        layout: { hierarchical: { enabled: false } },
        manipulation: {
            enabled: false, // Initial state, controlled by toggle
            async addNode(nodeData, callback) {
                const label = prompt("Enter node label:", "NewLabel");
                if (label === null) { callback(null); return; } // Cancelled
                const payload = { label, properties: {} };
                try {
                    const newNode = await apiRequest(API_ENDPOINT_NODE, 'POST', payload);
                    const visNode = { ...newNode, label: newNode.label, original_label: newNode.label };
                    callback(visNode);
                } catch (error) {
            ...
            async editNode(nodeData, callback) { openNodeEditModal(nodeData, callback); },
            async addEdge(edgeData, callback) {
                const label = prompt("Enter edge label (optional):", "");
                if (label === null) { callback(null); return; } // Cancelled
                const payload = { from_node_id: edgeData.from, to_node_id: edgeData.to, label, properties: {} };
                try {
                    const newEdge = await apiRequest(API_ENDPOINT_EDGE, 'POST', payload);
                    const visEdge = { ...newEdge, from: newEdge.from_node_id, to: newEdge.to_node_id };
                    callback(visEdge);
                } catch (error) {
            ...
            async editEdge(edgeData, callback) { openEdgeEditModal(edgeData, callback); },
            async deleteNode(data, callback) {
                if (confirm(`Delete ${data.nodes.length} nodes and their connected edges?`)) {
                    try {
                        for (const nodeId of data.nodes) await apiRequest(`${API_ENDPOINT_NODE}${nodeId}`, 'DELETE');
                        callback(data);
                    } catch (error) { alert(`Failed to delete node: ${error}`); callback(null); }
                } else callback(null);
            },
            async deleteEdge(data, callback) {
                if (confirm(`Delete ${data.edges.length} edges?`)) {
                    try {
                        for (const edgeId of data.edges) await apiRequest(`${API_ENDPOINT_EDGE}${edgeId}`, 'DELETE');
                        callback(data);
                    } catch (error) { alert(`Failed to delete edge: ${error}`); callback(null); }
                } else callback(null);
            },
        }
    };
    ...
    network.on("doubleClick", (params) => {
        if (editModeToggle.checked) {
            if (params.nodes.length > 0) openNodeEditModal(nodesDataSet.get(params.nodes[0]), (data) => nodesDataSet.update(data));
            else if (params.edges.length > 0) openEdgeEditModal(edgesDataSet.get(params.edges[0]), (data) => edgesDataSet.update(data));
        }
    });
}
...
editModeToggle?.addEventListener("change", (e) => {
    ...
    editModeHint.textContent = isChecked ? "Toolbar active. Double-click to edit." : "Allows direct graph manipulation.";
...
fuseEntitiesBtn?.addEventListener("click", async () => {
    if (!sessionId) { showUserStatus(graphActionStatus, "Cannot fuse: not connected.", "error"); return; }
    if (!confirm("This may take time. Continue?")) return;
    showUserStatus(graphActionStatus, "Starting entity fusion...", "info", 0);
    graphLoadingOverlay.classList.remove('hidden');
    try {
        const response = await apiRequest(API_ENDPOINT_GRAPH_FUSE, 'POST', { sid_body: { sid: sessionId } });
        const data = await response.json();
        showUserStatus(graphActionStatus, `Fusion started (Task: ${data.task_id}).`, "success", 10000);
    } catch (error) { showUserStatus(graphActionStatus, `Failed to start fusion: ${error.message}`, "error"); }
    finally { graphLoadingOverlay.classList.add('hidden'); }
});
findPathForm?.addEventListener("submit", async (e) => {
    e.preventDefault();
    const startId = parseInt(startNodeInput.value, 10), endId = parseInt(endNodeInput.value, 10);
    if (isNaN(startId) || isNaN(endId)) { showUserStatus(graphActionStatus, "Valid Node IDs required.", "warning"); return; }
    ...
    try {
        ...
        network.selectNodes(nodeIdsInPath); network.selectEdges(edgeIdsInPath);
        network.fit({ nodes: nodeIdsInPath, animation: true });
        showUserStatus(graphActionStatus, `Path found.`, "success");
    } catch (error) { showUserStatus(graphActionStatus, `Pathfinding failed: ${error.message}`, "error"); }
    finally { graphLoadingOverlay.classList.add("hidden"); }
});
expandNeighborsBtn?.addEventListener("click", async (e) => {
    const nodeId = parseInt(e.currentTarget.dataset.nodeId, 10);
    if (isNaN(nodeId)) return;
    showUserStatus(graphActionStatus, `Expanding neighbors...`, "info", 0);
    graphLoadingOverlay.classList.remove("hidden");
    try {
        ...
        nodesDataSet.update(newNodes); edgesDataSet.update(newEdges);
        showUserStatus(graphActionStatus, `Added ${newNodes.length} neighbors.`, "success");
    } catch (error) { showUserStatus(graphActionStatus, `Failed to expand: ${error.message}`, "error"); }
    finally { graphLoadingOverlay.classList.add("hidden"); }
});
function applyCurrentLayout() {
    ...
    graphLoadingOverlay.classList.remove('hidden');
    let layoutOptions = { hierarchical: { enabled: false } }, physicsEnabled = true;
    switch (currentLayoutMethod) {
        case "hierarchicalRepulsion":
            ...
            network.setOptions({ physics: { solver: 'hierarchicalRepulsion' }}); break;
        case "hierarchicalDirected":
            layoutOptions.hierarchical = { enabled: true, direction: "LR", sortMethod: "directed", levelSeparation: 220 };
            physicsEnabled = false; break;
        default: network.setOptions({ physics: { solver: 'barnesHut' }}); break;
    }
    network.setOptions({ layout: layoutOptions, physics: { enabled: physicsEnabled } });
    if (!physicsEnabled) network.stabilize();
    appSettings.physicsOnLoad = physicsEnabled; updatePhysicsButtonText();
    if(settingPhysicsOnLoad) settingPhysicsOnLoad.checked = physicsEnabled;
    localStorage.setItem('graphExplorerSettings', JSON.stringify(appSettings));
    setTimeout(() => { network.fit(); graphLoadingOverlay.classList.add('hidden'); showUserStatus(graphActionStatus, `Layout applied.`, "success"); }, 100);
}
...
function addChatMessage(message, sender, isThinking = false) {
    ...
    if (isThinking) { messageDiv.innerHTML = `<i class="fas fa-spinner fa-spin"></i>`; messageDiv.id = "thinking-indicator"; }
    else if (sender === 'ai') { messageDiv.innerHTML = markdownConverter.makeHtml(message); }
...
async function handleChatSubmit(event) {
    ...
    if (!query) return;
    addChatMessage(query, 'user'); chatInput.value = '';
