...
# =====================================================================
# LINE 1: IMMEDIATE UNBUFFERED DIAGNOSTIC OUTPUT
# =====================================================================
print("[DIAGNOSTIC 1/5] shared_vectorizer_usage.py starting execution...", flush=True)
...
print(f"[DIAGNOSTIC 2/5] Standard modules loaded (Python {sys.version.split()[0]} on {sys.platform})", flush=True)
...
print("[DIAGNOSTIC 3/5] PyTorch/torchvision environment sanitized.", flush=True)

print("[DIAGNOSTIC 4/5] Importing safe_store...", flush=True)
try:
    import safe_store
    from safe_store import SafeStore, LogLevel, shutdown_shared_vectorizer
    print("[DIAGNOSTIC 5/5] safe_store successfully imported!", flush=True)
except Exception as e:
    print(f"[!] FATAL: Failed to import safe_store: {e}", flush=True)
    import traceback
    traceback.print_exc()
    sys.exit(1)
